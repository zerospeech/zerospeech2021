...
from zerospeech2021 import lexical, syntactic
...
def main():
    ...
    # syntactic
    by_pair, by_type = syntactic.evaluate(
        args.gold_file, args.submission_file)

    by_pair.to_csv(
        args.output_directory / 'score_syntactic_by_pair.csv',
        index=False, float_format='%.4f')
    by_type.to_csv(
        args.output_directory / 'score_syntactic_by_type.csv',
        index=False, float_format='%.4f')
